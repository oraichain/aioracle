...
const verify = async (contractAddr, listProofs) => {
  for (let proof of listProofs) {
    const input = toBinary({
      verify_data: {
        stage: parseInt(requestId),
        data: toBinary(proof.finalReport),
        proof: proof.proofs.proofs
      }
    });
    const result = await fetch(`${process.env.LCD_URL}/cosmwasm/wasm/v1/contract/${contractAddr}/smart/${input}`).then((data) => data.json());
