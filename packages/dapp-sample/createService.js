...
const { AioracleContractClient } = require('@oraichain/aioracle-contracts-sdk/src');
...
const demo = async () => {
  ...
  const { sender, client } = await getClientAndSender(mnemonic);
  const contract = new AioracleContractClient(client, sender.address, contractAddr);
