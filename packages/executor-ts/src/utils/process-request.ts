...
export const processRequest = async (requestId: number, mnemonic: string, isAwait = false) => {
  ...
  const aioracleClient = new AioracleContractClient(client, executor, contractAddr);
...
const processData = async ({
  ...
  // get service contracts to get data from the scripts, then submit report
  return executeService(requestId, serviceName, input, aioracleClient).then(async (executeResult) => {
    const leaf: Leaf = {
      executor,
    ...
    const { submitted } = await checkSubmit(aioracleClient.contractAddress, requestId, executor);
    if (!submitted) {
