...
export class IntervalService {
  ...
  async submitReportInterval(gasPrices: number) {
    ...
    const msgs: AioracleContractTypes.ExecuteMsg[] = []; // msgs to broadcast to blockchain network
    ...
    for (let { requestId, threshold } of queryResult) {
      ...
      console.log('request id with report count and threshold: ', {
        requestId,
        reportCount,
        threshold,
      });
      ...
      if (reportCount === threshold) {
        ...
        const reports = await this.repoExec.queryExecutorReportsWithThreshold(
          requestId,
          threshold,
        );
        ...
        let root = data.merkle_root || newRoot;
        ...
        const { submitted } = await this.repoRequ.findSubmittedRequest(
          requestId,
        );
        if (submitted) {
          const removeResult = await this.repoRequ.removeRedundantRequests(
            requestId,
          );
        ...
        const removeResult = await this.repoExec.updateReports(
          requestId,
          numRedundant,
        );
    ...
    if (msgs.length > 0) {
      ...
      // query latest block
      await this.processUnsubmittedRequests(msgs, gasPrices, requestsData);
    }
  }
  ...
  async processSubmittedRequest(
    ...
    try {
      ...
      const merkleRoot = await this.repoMerk.findMerkleRoot(
        submittedMerkleRoot,
      );
      if (!merkleRoot) {
        const insert = await this.repoMerk.insertMerkleRoot(
          submittedMerkleRoot,
          leaves,
        );
  ...
  async processUnsubmittedRequests(
    msgs: AioracleContractTypes.ExecuteMsg[],
    gasPrices: number,
