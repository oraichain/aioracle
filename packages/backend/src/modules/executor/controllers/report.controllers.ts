...
export class ReportController {
  ...
  async checkSubmit(
    ...
    const report = await repo.findReport(query.request_id, query.executor);
    if (!report) {
      return {
  ...
  async getReports(
    ...
    const pagerNumber = paginatorNumber(
      query.page_number,
    ...
    return res.status(HttpStatus.NOT_FOUND).json({
      message:
        'cannot find the reports with the given request id and contract address',
    });
  }
  ...
  async getReport(
    ...
    return res.status(HttpStatus.NOT_FOUND).json({
      message:
        'cannot find the report with the given request id, contract address & executor',
    });
  }

  @Post('/')
  async submitReport(@Res() res: Response, @Body() body: ReportPost) {
    const repo = new ExecutorRepository();
    ...
    const threshold = requestData.threshold;
    // verify executor not in list
    const verifyContract = await this.executorService.isWhiteListed(
      config.CONTRACT_ADDRESS,
    ...
    if (
      !this.executorService.verifySignature(
    ...
    const executorReport = await repo.findReport(
      body.request_id,
      body.report.executor,
    );
    ...
    if (reportCount < threshold) {
      ...
      try {
        ...
        await repo.insertExecutorReport(
          body.request_id,
          body.report.executor,
          body.report,
        );
