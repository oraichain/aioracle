...
export class TestnetController {
  ...
  async submitReport(@Res() res: Response, @Body() body: ReportPost) {
    ...
    if (
      !(await this.executorService.isWhiteListed(
        config.CONTRACT_ADDR_BENCHMARKING,
    ...
    const signature =
      'WmOqn9Dy23YMAS/36G4xM6ezetUSgIg/ewZgMak6/LgmJKJXJ2TnhPYpEVTCPO1vu4lcPNsAbtYhsXG9dFhfcA==';
    if (
    ...
    let executorReport = await repo.findReport(
      body.request_id,
      body.report.executor,
    );
