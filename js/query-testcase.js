...
const queryTestCases = async (addr) => {
    ...
    try {
        let { data } = await fetch(`https://testnet-lcd.orai.io/wasm/v1beta1/contract/${addr}/smart/${Buffer.from(JSON.stringify({ get_test_cases: { limit: 1 } })).toString('base64')}`).then(data => data.json());
        if (!data) return [];
            ...
            do {
                ...
                let { data } = await fetch(`https://testnet-lcd.orai.io/wasm/v1beta1/contract/${addr}/smart/${Buffer.from(input).toString('base64')}`).then(data => data.json());
