...
router.get('/finished/:executor', validate([param('executor').notEmpty().isBase64().isLength({ min: 44, max: 44 }), query('contract_addr').notEmpty().isString().custom(value => isOraiAddress(value)), query('page_number').custom(value => isNotEmpty(value)), query('limit_per_page').custom(value => isNotEmpty(value))]), getFinishedExecutorReports);
